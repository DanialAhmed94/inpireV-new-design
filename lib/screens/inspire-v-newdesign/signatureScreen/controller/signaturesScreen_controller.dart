...
enum ImageUploadStatus1 { initial, uploading, success }

class InspireVSignatureController extends BaseController {
  ...
  ImageUploadStatus1 imageUploadStatus = ImageUploadStatus1.initial;
  ...
  imageUpload({required String imagePath, required String signType}) async {
    imageUploadStatus = ImageUploadStatus1.uploading;
    update();
    var response =
    await deficienciesInsideRepository.getImageUpload(filePath: imagePath);
    response.fold((l) {
      imageUploadStatus = ImageUploadStatus1.initial;
      return null;
    }, (r) {
      if (signType == "tenant") {
      ...
      imageUploadStatus = ImageUploadStatus1.success;
    });
    update();
  }
